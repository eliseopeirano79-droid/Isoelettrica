...
root.IsoPathologyUI={create(api,lab){
 ...
 function preview(){
  clearMarker();lab.setCongenital(null);if(!current)return;
  ...
  if(current.preview==='structural'){const d=root.CongenitalAnatomy.definitions[current.morphology];if(current.morphology==='bicuspid')overlay['valve.aortic.open']=params.shape??d.def;lab.clinicalOverlay(overlay);lab.setCongenital(current.morphology,params.shape??d.def);api.setView(d.view);api.selectPart(null);return;}
 ...
 function render(){detail.replaceChildren();if(!current){line('Scegli un quadro per vedere il meccanismo disponibile.');return;}
  line(current.name,'h3');line({structural:'Morfologia 3D illustrativa · modificabile',ecg:'ECG e conduzione sincronizzati',localization:'Localizzazione · geometria patologica da sviluppare',duct:'Dotto pervio · calibro modificabile','valve-stenosis':'Lembi animati · apertura limitata','valve-regurgitation':'Lembi animati · coaptazione incompleta'}[current.preview],'span','pathology-status');line(current.description);
  ...
  if(current.preview==='structural'){const d=root.CongenitalAnatomy.definitions[current.morphology];numberControl(d.control,'shape',params.shape??d.def,d.min,d.max,.05,'×');line('Ricostruzione geometrica semplificata sul cuore di riferimento. Posizione e proporzioni sono illustrative: pressioni, shunt e saturazioni non sono calcolati.','p','pathology-limit');const focus=document.createElement('button');focus.textContent='Osserva la malformazione';focus.onclick=()=>current.morphology==='bicuspid'?api.focusValve('aortic'):api.focusStructure(d);detail.append(focus);if(current.morphology==='iaa')line('Mostra una variante illustrativa di tipo B: discontinuità fra carotide comune e succlavia sinistre.');}
  if(current.preview==='duct'){numberControl('Diametro del dotto','diameter',params.diameter??4,1,12,.5,'mm');const focus=document.createElement('button');focus.textContent='Osserva il Botallo';focus.onclick=()=>api.focusDuct();detail.append(focus);}
 ...
 function reset(){current=null;params={};choice.value='';clearMarker();lab.setCongenital(null);lab.clearClinicalOverlay();api.clearClinicalCase();api.setView('surface');api.selectPart(null);render();}
