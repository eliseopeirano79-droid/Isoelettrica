...
window.addEventListener('message',e=>{if(e.source!==window.parent||e.origin!==location.origin)return;if(e.data?.type==='iso-lab-visibility'){window.IsoLabEmbedded.active=e.data.active===true;document.documentElement.dataset.theme=e.data.theme==='light'?'light':'dark';window.dispatchEvent(new Event('iso-theme'));}});
