...
// Average coincident seam normals within the vascular assembly and reconstructed atrial ostia.
// Separate labels/materials and open vessel ends are preserved.
function seamNormals(meshes){const buckets=new Map();for(const m of meshes.filter(m=>m.userData.layer==='vessels'||m.userData.refined&&['Left atrium','Right atrium','Right ventricle'].includes(m.userData.sourceName))){const p=m.geometry.attributes.position,n=m.geometry.attributes.normal;if(!n)continue;for(let i=0;i<p.count;i++){const key=[p.getX(i),p.getY(i),p.getZ(i)].map(x=>Math.round(x*1e4)).join(',');if(!buckets.has(key))buckets.set(key,[]);buckets.get(key).push({m,i,n});}}
...
function semilunarPoint(rig,leaf,u,v,open){
 const r=rig.radius,angle=(leaf+u)*Math.PI*2/rig.count,a0=leaf*Math.PI*2/rig.count,a1=(leaf+1)*Math.PI*2/rig.count;
 const radial=a=>rig.axis.clone().multiplyScalar(Math.cos(a)).addScaledVector(rig.side,Math.sin(a));
 const hinge=radial(angle).multiplyScalar(r),height=.55*r*Math.cos(Math.PI*u)**2;
 // Scalloped crown attachment; free margins share the same coaptation rays.
 const edge=radial(u<=.5?a0:a1).multiplyScalar(r*Math.abs(2*u-1));
 const opened=radial(angle).multiplyScalar(r*(1-.16*Math.sin(Math.PI*u)));
 edge.lerp(opened,open);const edgeHeight=.55*r-.07*r*Math.sin(Math.PI*u)*(1-open);
 return rig.center.clone().add(hinge.lerp(edge,v)).addScaledVector(rig.n,height*(1-v)+edgeHeight*v+.13*r*Math.sin(Math.PI*v)*Math.sin(Math.PI*u));
}
function valvePoint(rig,leaf,u,v,open){if(!rig.pap)return semilunarPoint(rig,leaf,u,v,open);const angle=(leaf+u)*Math.PI*2/rig.count,rad=rig.radius*(1-v+.8*open*v);return rig.center.clone().addScaledVector(rig.axis,rad*Math.cos(angle)).addScaledVector(rig.side,rad*Math.sin(angle)).addScaledVector(rig.n,-rig.radius*(.1*Math.sin(Math.PI*v)+.9*open*v));}
...
return {nearestPoint,seamNormals,pick,valvePoint,semilunarPoint,constrainedLeaf};
});
