...
function seamNormals(meshes){const buckets=new Map();for(const m of meshes.filter(m=>m.userData.layer==='vessels'||m.userData.refined&&m.userData.sourceName==='Left atrium')){const p=m.geometry.attributes.position,n=m.geometry.attributes.normal;if(!n)continue;for(let i=0;i<p.count;i++){const key=[p.getX(i),p.getY(i),p.getZ(i)].map(x=>Math.round(x*1e4)).join(',');if(!buckets.has(key))buckets.set(key,[]);buckets.get(key).push({m,i,n});}}
