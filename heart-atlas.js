...
const motion=`uniform float uAtlasA;uniform float uAtlasV;uniform float uAtlasRegion;
vec3 atlasMotion(vec3 p){float a=uAtlasRegion>1.5?0.0:(uAtlasRegion>.5?1.0:smoothstep(-.05,.65,p.y));float base=1.0-smoothstep(.55,1.15,p.y);float v=(1.0-a)*uAtlasV*base;float at=a*uAtlasA*base;vec3 c=vec3(.15,-.32,.2),d=p-c;float angle=v*.087,cs=cos(angle),sn=sin(angle);d.xz=mat2(cs,-sn,sn,cs)*d.xz;d.xz*=1.0-.10*v-.06*at;d.y*=1.0-.04*v-.025*at;return c+d;}`;
function build(options={}){
 const group=new T.Group();group.scale.setScalar(SCALE);const seg={},coro={},meshes=[],uniforms={uAtlasA:{value:0},uAtlasV:{value:0}};let opacity=options.opacity??1;
 ...
 function add(m){meshes.push(m);group.add(m);if(!options.coronary){const region=/atrium/.test(m.name)?1:/ventricle/.test(m.name)?2:0;m.material.onBeforeCompile=s=>{Object.assign(s.uniforms,uniforms,{uAtlasRegion:{value:region}});s.vertexShader=motion+'\n'+s.vertexShader;s.vertexShader=s.vertexShader.replace('#include <begin_vertex>','vec3 transformed=atlasMotion(position);');};m.material.customProgramCacheKey=()=> 'atlas-ecg-motion-v1';}return m;}
 function movingMaterial(mat){mat.onBeforeCompile=s=>{Object.assign(s.uniforms,uniforms,{uAtlasRegion:{value:0}});s.vertexShader=motion+'\n'+s.vertexShader;s.vertexShader=s.vertexShader.replace('#include <begin_vertex>','vec3 transformed=atlasMotion(position);');};mat.customProgramCacheKey=()=> 'atlas-conduction-motion';return mat;}
 function deformPoint(p){const a=p.y<=-.05?0:p.y>=.65?1:(()=>{const x=(p.y+.05)/.7;return x*x*(3-2*x);})(),x=Math.min(1,Math.max(0,(p.y-.55)/.6)),base=1-x*x*(3-2*x),v=(1-a)*uniforms.uAtlasV.value*base,at=a*uniforms.uAtlasA.value*base,d=p.clone().sub(new T.Vector3(.15,-.32,.2)),angle=v*.087,dx=d.x,dz=d.z;d.x=(Math.cos(angle)*dx+Math.sin(angle)*dz)*(1-.1*v-.06*at);d.z=(-Math.sin(angle)*dx+Math.cos(angle)*dz)*(1-.1*v-.06*at);d.y*=1-.04*v-.025*at;return d.add(new T.Vector3(.15,-.32,.2));}
 const out={group,seg,coro,rv,movingMaterial,deformPoint,setTissueOpacity(v,xray=false){for(const m of meshes){const vessel=m.userData.layer==='coronaries';if(!vessel){m.material.opacity=v;m.material.transparent=v<1;m.material.depthWrite=v===1;}else{m.material.depthTest=!xray;m.renderOrder=xray?4:0;}}},SEG:ISO_CUORE.SEG,meshes,loaded:false,setOpacity(v){opacity=v;for(const m of meshes){m.material.opacity=v;m.material.transparent=v<1;m.material.depthWrite=v===1;}},update(t,stream,cfg){const clock=window.CardiacClock?CardiacClock.read(stream,t,cfg):null,e=clock?clock.events:stream.eventsAround(t),a=e.A?t-e.A.t:Infinity,v=e.V?t-e.V.t:Infinity;const pulse=(x,start,d)=>x>=start&&x<start+d?Math.sin(Math.PI*(x-start)/d):0;const organized=!['vf','torsade'].includes(cfg.cont)&&cfg.mode!=='asystole';uniforms.uAtlasA.value=cfg.cont==='af'||cfg.cont==='flutter'?0:pulse(a,35,140);uniforms.uAtlasV.value=organized?pulse(v,40,300):0;}};
 out.ready=load().then(([gltf,map])=>{
  ...
  out.setOpacity(opacity);out.loaded=true;return out;
 });return out;
}
