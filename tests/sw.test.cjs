...
test('Installazione completa: tutte le risorse richieste sono in cache',async()=>{
  ...
  const c=await w.caches.open('isoelettrica-v42.0');
  for(const f of ['index.html','quiz.js?v=42.0','engine.js?v=42.0','app.js?v=42.0','anatomia.html']) assert.ok(await c.match(base+f),f);
});
test('Un download essenziale fallito annulla l’installazione e preserva la versione precedente',async()=>{
  const w=worker(async req=>new Response('x',{status:String(req).endsWith('quiz.js?v=42.0')?503:200}));
  await w.put('isoelettrica-v39','index.html','old');
  await assert.rejects(w.dispatch('install'),/indispensabile/);
  assert.ok(w.maps.has('isoelettrica-v39'));assert.equal(w.maps.has('isoelettrica-v42.0'),false);
});
...
test('Offline: pagina da cache, script assente mai sostituito da HTML',async()=>{
  const w=worker(async()=>{throw Error('offline');});await w.put('isoelettrica-v42.0','index.html','<html>offline</html>');
...
test('HTTP 503 usa la copia disponibile e la shell non mescola versioni',async()=>{
  const w=worker(async()=>new Response('unavailable',{status:503}));await w.put('isoelettrica-v42.0','aux.js','cached');
  const response=await w.dispatch('fetch',{request:new Request(base+'aux.js')});assert.equal(await response.text(),'cached');
  await w.put('isoelettrica-v42.0','app.js?v=42.0','version40');const calls=w.calls.length;
  const core=await w.dispatch('fetch',{request:new Request(base+'app.js?v=42.0')});assert.equal(await core.text(),'version40');assert.equal(w.calls.length,calls);
});
...
test('Ripristino elimina solo le cache Isoelettrica e conferma dopo il completamento',async()=>{
  const w=worker();await w.put('isoelettrica-v42.0','index.html','x');await w.put('isoelettrica-assets-v1','atlante/a.jpg','x');await w.put('foreign-cache','index.html','y');
